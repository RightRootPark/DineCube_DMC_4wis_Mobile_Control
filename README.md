...
로봇은 **TCP 서버 모드**로 동작하며, 외부 클라이언트로부터 `X, Y, C(Yaw)` 명령을 수신하여 주행 모드(Spot Turn, Crab, 4WS)를 자동으로 전환 및 제어합니다.

## 📌 주요 기능 (Features)

*   **TCP Server 통신**: 포트 `5009`를 통해 외부 제어기와 통신.
*   **자동 모드 전환**: 입력된 벡터(x, y, c)에 따라 주행 모드 자동 판단.
    *   **Spot Turn**: 제자리 회전 (마름모 조향)
    *   **Crab**: 게걸음 주행 (전/후륜 동일 조향)
    *   **4WS**: 일반 조향 (전/후륜 역위상)
*   **안전 기능**: 통신 단절 시 자동 정지 및 재접속 대기, 절댓값 연산을 위한 안전 로직 구현.
...
*   **Data Format**: ASCII String (CSV)
    ...
    *   `y`: 횡이동 속도/방향
    *   `c`: 회전(Yaw) 속도/조향각
    *   *예시: `100.5,0,0` (전진), `0,50.2,30.0` (복합 동작)*
